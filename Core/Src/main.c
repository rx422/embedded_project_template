...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	HAL_UART_Transmit(&huart1, (uint8_t*)output_data_buff, strlen(output_data_buff), HAL_MAX_DELAY);
	HAL_UART_Receive(&huart1, (uint8_t*)input_data_buff, 2, HAL_MAX_DELAY);
	printf("%s\n", input_data_buff);
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
		...
		if (__HAL_UART_GET_FLAG(&huart1, UART_FLAG_RXNE) != RESET)
		{
			...
			HAL_UART_Receive(&huart1, (uint8_t*) input_data, 2, HAL_MAX_DELAY);

			switch (input_data[0])
			{
				case 'L':
					motorControl(&motor, "motorLeft");
					break;
				case 'R':
					motorControl(&motor, "motorRight");
					break;
				case 'S':
					motorControl(&motor, "motorStop");
					break;
				default:
